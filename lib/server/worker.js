...
server = http2.createSecureServer(Object.assign(config.server, utils.set_cert()));
...
server.on('stream', function (stream, headers, flags) {
  ...
  // ensure http2 only | fallback to request
  ...
  if(alpn === 'h2' || alpn === 'h2c'){
    ...
    console.log(session.socket.alpnProtocol);
...
/*
server.on('unknownProtocol', function (req, res) {
  console.log(req)
//  res.send('ok');
