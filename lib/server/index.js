...
for (let i = 0; i < config.stream.methods.length; i++) {
  evts[config.stream.methods[i]] = new evt();
  router[config.stream.methods[i]] = function(dest_url,stream,headers,flags){
    return evts[config.stream.methods[i]].on(dest_url,stream,headers,flags);
  }
}
...
for (let i = 0; i < engine.engines.length; i++) {
  if(engine[engine.engines[i]].enabled){
    ...
    console.log('Template engine set to '+ engine.engines[i])
    break;
  }
}
engine = null;
...
for (let i = 0; i < cache_arr.length; i++) {
  if(config[cache_arr[i]].cache.enabled){
    console.log(cache_arr[i] +' cache watching...')
...
server.on('listening', function(err,res){
  utils.cc(['init', 'Server listening at https:\/\/localhost:'+config.port],96);
})
...
server.on('stream', function (stream, headers,flags) {
  ...
  try {
    ...
    if(config.stream.methods.indexOf(method) === -1){
    ...
    if(ext !== '' && method === 'get'){
      ...
      try {
        ...
        evts[method].emit(x.pathname, stream, headers, flags);
      ...
      stream.on('end', function(){
        ...
        try {
          ...
          evts[method].emit(dest_url, stream, headers, flags);
          body = null;
