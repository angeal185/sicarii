...
const init = {
  config: function(){
    ...
    fs.writeFileSync(cwd + '/config/config.json', JSON.stringify(config))
    cx(['init', dir + ' created'],96);
    dir = '/config/ip_config.json';
    fs.writeFileSync(cwd + '/config/ip_config.json', JSON.stringify({blacklist: [],whitelist:[]}))
    cx(['init', dir + ' created'],96);
    process.exit(1)
  },
  build: function(){
    ...
    fs.writeFileSync(cwd + dir, base_html)
    cx(['init', dir + ' created'],96);

    dir = '/static/modules/main.mjs';
    fs.writeFileSync(cwd + dir, base_js)
    cx(['init', dir + ' created'],96);

    dir = '/static/css/main.css';
    fs.writeFileSync(cwd + dir, '')
    cx(['init', dir + ' created'],96);
