...
const utils = {
  ...
  store_assign: function(src, arr, x){
    try {
      ...
      exists = false,
      msg = {success: false, msg: src+ ' assign error'};

      for (let i = 0; i < items.length; i++) {
        if(items[i][item] === arr[0][item]){
          x[src][i] = Object.assign(items[i], arr[1]);
          exists = true;
          break;
        }
      }
  ...
  store_each: function(src, obj, x){
    try {
      ...
      let items = x[src],
      msg = {success: false, msg: src+ ' each error'};

      for (let i = 0; i < items.length; i++) {
        items[i] = Object.assign(items[i], obj);
      }
  ...
  store_omit: function(src, arr, x){
    try {
      ...
      let items = x[src],
      msg = {success: false, msg: src+ ' omit error'};

      for (let i = 0; i < items.length; i++) {
        for (let j = 0; j < arr.length; j++) {
          delete items[i][arr[j]]
        }
      }
      msg.success = true;
  ...
  store_filter: function(src, obj, x){
    try {
      ...
      data.data = items.filter(function(i){
  ...
  store_findIndex: function(src, obj, x){
    try {
      ...
      data.data = items.findIndex(function(i){
        return i[item] === obj[item]
      })
  ...
  store_import: function(src, obj, x, cb){
  ...
  store_export: function(src, obj, x, cb){
    ...
    if(!cb){
      try {
        fs.writeFileSync(obj.dest, JSON.stringify(x[src]));
      } catch (err){
        msg.success = false;
        msg.msg = src +' export failed';
      } finally{
        return msg;
      }
    }
  ...
  store_sort: function(src, obj, x){
    try {
      ...
      data.data = utils.sortBy(items, obj.key)
