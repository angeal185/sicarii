        ...
        for (let i in mtype) {
          if(i === obj.ctype && mtype[i].indexOf(ext) !== -1){
            ...
            fs.mkdir(base + dest, {recursive: config.uploads.recursive}, function(err){
              ...
              if(gz){
                obj.path += config.gzip.ext;
                obj.data = zlib.gzipSync(obj.data, config.gzip.settings);
              } else if(br){
                obj.path += config.brotli.ext;
                obj.data = zlib.brotliCompressSync(obj.data, config.brotli.settings);
              }
  ...
  serve: function(src, url, ctype, stream, cache){
    ...
    utils.cache_stream(config.cache.url, obj, function(err,cached){
      ...
      try {
        ...
        } else if(config.brotli.enabled && config.brotli.prezipped){
          file += config.brotli.ext
        }
