  ...
  decrypt: function(encdata, key, cb) {
    try {
      ...
      if(['gcm', 'ocb', 'ccm'].indexOf(defaults.mode) !== -1){
        ...
        final = decipher.update(encdata.slice(tag_slice), 'binary', 'utf8') + decipher.final('utf8');
      } else {
        final = decipher.update(encdata.slice(defaults.iv_len), 'binary', 'utf8') + decipher.final('utf8');
      }
