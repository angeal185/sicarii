...
function render_tpl(stream, file, data,  partials, url, cb){
  fs.readFile(file, 'utf8', function(err, content) {
    if(err){
      utils.err(stream, 'GET', url, 404, 'mustache template Found');
      if(cb){cb(err)}
      return;
    }
    try {
      ...
      utils.err(stream, 'GET', url, 500, 'mustache template render error');
      if(cb){cb(err)}
      return;
    }
  ...
  if(tpl_count && typeof data.partials === 'object'){
    ...
    for (let i in data.partials) {
      ...
      fs.readFile(render_path + data.partials[i], 'utf8', function(err,res){
        if(err){
          cnt = 9999;
          utils.err(stream, 'GET', url, 500, 'mustache template include error');
          if(cb){cb(err)}
          return;
        }
        tpl_obj[i] = res;
